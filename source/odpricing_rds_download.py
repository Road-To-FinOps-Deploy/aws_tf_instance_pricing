...
def lambda_handler(event, context):
    ...
    try:
        ...
        r = http.request('GET', 'https://pricing.us-east-1.amazonaws.com/offers/v1.0/aws/AmazonRDS/current/index.csv')
        ...
        for line in plaintext_lines:
            ...
            if ((str(line).find('OnDemand') != -1) and (str(line).find('CreateDBInstance') != -1)):
                pricing_output += str(line.decode("utf-8"))
                pricing_output += "\n"
        #removing additional commer    
        if str(pricing_output.find("Up to 10,000 Mbps")) != -1:   
            pricing_output = str(pricing_output.replace("Up to 10,000 Mbps", "Up to 10000 Mbps"))
        # Add the output to a local temporary file & zip it
        with gzip.open('/tmp/od_pricedata_rds.txt.gz', 'wb') as f:
        ...
        s3.meta.client.upload_file('/tmp/od_pricedata_rds.txt.gz', os.environ["BUCKET_NAME"], 'Pricing/od_pricedata_rds/od_pricedata_rds.txt.gz')
